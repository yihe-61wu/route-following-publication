...
def compute_avg_path_with_range_interpolation(csv_files, folder_path):
    ...
    # Compute the range (max and min) for each position
...
def plot_average_path_with_baseline_and_range(avg_x, avg_y, max_x, max_y, min_x, min_y, baseline_csv):
    ...
    # Plot the range using fill_between
    plt.fill_between(avg_x, min_y, max_y, color='lightgreen', alpha=0.3, label='Path Range')
    
    plt.plot(avg_x, avg_y, '-g', linewidth=3, label='Average Path')
    plt.plot(baseline_x, baseline_y, '--b', linewidth=3, label='Baseline Path')
    plt.xlim(-0.5, 3.5)
    plt.ylim(-1.8, 1.8) 
    plt.title('Average Odom Path Visualization with Baseline and Range')
    plt.xlabel('X(m)')
    plt.ylabel('Y(m)')
    plt.legend()
    plt.grid(True)
...
# Paths
folder_path = "/Users/hakosaki/Desktop/DataProcessing/left_day2_afternoon/successed"
csv_files = [f for f in os.listdir(folder_path) if f.endswith('.csv')]
baseline_csv = "/Users/hakosaki/Desktop/DataProcessing/left_day2_afternoon/baseline.csv"

# Compute and plot
avg_x, avg_y, max_x, max_y, min_x, min_y = compute_avg_path_with_range_interpolation(csv_files, folder_path)
plot_average_path_with_baseline_and_range(avg_x, avg_y, max_x, max_y, min_x, min_y, baseline_csv)
